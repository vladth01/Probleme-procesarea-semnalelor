...
# Inițializare variabile pentru p și q optime
best_aic = np.inf
best_p = 0
best_q = 0

# Calculul parametrilor optimi p și q folosind ARIMA
for p in range(21):  # Range-ul pentru p și q este de la 0 la 20 inclusiv
    for q in range(21):
        try:
            model = sm.tsa.ARIMA(serie_timp, order=(0, 0, q))  # Model MA - ordinea (0, 0, q)
            results = model.fit()
            aic = results.aic
            
            # Actualizăm parametrii optimi dacă găsim o valoare AIC mai mică
            if aic < best_aic:
                best_aic = aic
                best_p = 0
                best_q = q
...
print(f"Cea mai bună configurație ARIMA pentru modelul MA este: p = {best_p}, q = {best_q} cu AIC: {best_aic}")